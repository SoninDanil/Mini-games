...
public class Easy {
    ...
    private static ChoiceEasy[] choice = ChoiceEasy.values();
    ...
    public static void main(String[] args) {
        while (true) {
            ...
            if (user.equalsIgnoreCase("exit")) {
                TableEasy.table(wins, losses, ties, totalGames);
                break;
            }
    ...
    public static void roundResult(String user) {
        ChoiceEasy userInput = ChoiceEasy.valueOf(user);
        int randomIndex = random.nextInt(choice.length);
        ChoiceEasy randomChoice = choice[randomIndex];
        System.out.println("Вы выбрали: " + userInput);
        System.out.println("Программа выбрали: " + randomChoice);
        String result = MechanicsEasy.calculation(userInput, randomChoice);
        switch (result) {
            case "win":
                wins++;
                totalGames++;
    ...
    private static boolean isValidChoice(String userInput) {
        for (ChoiceEasy choice : ChoiceEasy.values()) {
